...
class ControllerExtensionModuleSlaSeoPro extends Controller {
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['sla_seo_pro_status']) && $this->request->post['sla_seo_pro_status']
		...
		$allow_input = array(
			'sla_seo_pro_status' => 1,
			'sla_seo_pro_include_path' => 1,
			'sla_seo_pro_postfix' => '',
			'sla_seo_pro_postfix_slash' => 0,
			'sla_seo_pro_postfix_slash_product' => 0,
			'sla_seo_pro_short_url' => 0,
			'sla_seo_pro_prefix_category' => '',
			'sla_seo_pro_prefix_product' => '',
			'sla_seo_pro_utm' => "tracking
utm_medium
utm_source
utm_campaign
utm_content
...
gclid
utm_term",
			'sla_seo_pro_not_use_cache' => 0,
			'sla_seo_pro_seo_page' => 0 ,
			'sla_seo_pro_allow_utm' => 1,
			'sla_seo_pro_use_lang_prefix' => 0,
			'sla_seo_pro_lang_prefix' => array(),
			'sla_seo_pro_hreflang' => array(),
			'config_seo_url' => 0,
			'sla_seo_pro_use_lang_prefix_main' => $this->config->get('config_language_id'),
		);
		...
		$data['use_lang_prefix_mains'] = [];
		$data['use_lang_prefix_mains'][''] = 'Использовать языковые алиас';
		foreach ($data['languages'] as $language) {
			$data['use_lang_prefix_mains'][$language['language_id']] = 'Использовать ' . $language['code'] . ' как базовый';
	...
	public function install() {
		...
		if (file_exists($file)) {
			...
			require($file);
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('sla_seo_pro', $_);
		}
