...
//Версия модуля
define ('google_xml_VERSION', '1.8.7');

class ControllerExtensionFeedGoogleXml extends Controller {
...
//---- Config section ----
	...
	*/
	protected function getYmlHeader() {
		$this->load->model('extension/feed/google_xml');
	...
	//			$this->setCurrency($currency['code'], number_format($this->offers_currency_value/$currency['value'], 4, '.', ''));
		...
		$this->categories = $this->model_extension_feed_google_xml->getCategoryTree($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers, $this->is_main_category);
...
*/
	...
	protected function getYmlChunk($page = 0) {
		...
		if ($this->numpictures > 1) {
			...
			$product_images = $this->model_extension_feed_google_xml->getProductImages($this->numpictures - 1);
			//---- Дополнительные изображения товара ----
		}
		$all_attributes = $this->model_extension_feed_google_xml->getAttributes($this->config->get($this->CONFIG_PREFIX.'attributes'));
		...
		foreach ($this->model_extension_feed_google_xml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers, $customer_group, $product_rel || $product_accessory, $skip, $limit, $this->is_main_category) as $product) {
			$is_out = false;
			$data = array();
...
//				$data['cbid'] = 15;
			...
			if ($product['special'] && $product['special'] < $product['price']) {
				...
				$data['g:sale_price'] = $product['special'];
			...
			*/
			...
			//$qattr = $this->db->query("SELECT `text` FROM ".DB_PREFIX."product_attribute WHERE product_id='".$product['product_id']."' AND language_id='3' AND attribute_id=17")->row;
			$qpc = $this->db->query("SELECT `category_id` FROM ".DB_PREFIX."product_to_category WHERE product_id='".$product['product_id']."' AND main_category='1'")->row;
			$qcat = $this->db->query("SELECT `name` FROM ".DB_PREFIX."category_description WHERE category_id='".$qpc['category_id']."' AND language_id='3'")->row;
			/*if($_SERVER['REMOTE_ADDR'] == '109.86.229.153'){
				//var_dump($qattr['text']); exit;
				var_dump($qcat['name']); exit;
			}*/

			$data['g:id'] = $product['product_id'];
			$data['g:title'] = $this->getFieldValue($product, $this->config->get($this->CONFIG_PREFIX.'name_field'), 'name') . ' ' . $product['model'];
			//$data['g:feed_label'] = 'ru';
			//$data['custom_label_1'] = 'ru';
			//$data['g:brand'] = $product['manufacturer'];
			$data['g:brand'] = 'Sunshouse';
			$data['g:brand'] = 'Sun\'s House';

			$data['g:age_group'] = 'взрослые';
			$data['g:gender'] = 'мужской';
			$data['g:custom_label_0'] = $qcat['name'];
			$data['g:product_type'] = $qcat['name'];
			...
			*/
			...
			$data['param'] = array();
			$attributes = $this->model_extension_feed_google_xml->getProductAttributes($product['product_id']);
			$attr_text = array();
	...
	*/
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		if ($this->color_options)
			$coptions = $this->model_extension_feed_google_xml->getProductOptions($this->color_options, $product['product_id']);
		$soptions = array();
		if ($this->size_options)
			$soptions = $this->model_extension_feed_google_xml->getProductOptions($this->size_options, $product['product_id']);
	...
	*/
	...
	protected function setOffer($data) {
		...
		if(isset($data['g:sale_price'])) $data['g:sale_price'] = round($data['g:sale_price'], 2). ' ' . $data['currencyId'];
		$offer = array();
	...
	//	$attributes = array('id', 'type', 'available', 'bid', 'cbid', 'param', 'delivery-option', 'group_id', 'accessory', 'age');
		...
		$allowed_tags = array('g:id'=>1, 'g:title'=>1, /*'g:feed_label'=>1,*/ 'g:description'=>0, 'g:link'=>0, 'g:image_link'=>0, 'g:condition'=>0,'g:availability' =>0,'g:price'=>1, 'g:sale_price'=>0, 'g:brand'=>0, 'g:google_product_category'=>0, 'g:mpn'=>0, /*'g:product_type'=>0, */'g:color'=>0, 'g:size'=>0, 'g:item_group_id'=>0, 'g:additional_image_link'=>0, 'g:custom_label_0'=>1, 'g:age_group'=>1, 'g:gender'=>1, 'g:product_type'=>1, 'buyurl'=>0, 'oldprice'=>0, 'wprice'=>0, 'xCategory'=>0,  'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0, 'min-quantity'=>0, 'step-quantity'=>0, 'stock_quantity'=>0);
		switch ($type) {
			case 'vendor.model':
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0,  'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'weight'=>0, 'downloadable'=>0, 'adult'=>0, /*'age'=>0,*/ 'barcode'=>0, 'keywords'=>0, 'rec'=>0));
