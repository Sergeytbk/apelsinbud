...
class ModelExtensionShippingCs extends Model {
	function getQuote($address) {
		...
		$textarray = $this->config->get('shipping_cs_group_shipping');
		$quote_data = array();
			...
			foreach($custom_shippings['shipping_cs'] as $key => $custom_shipping){
				...
				if ($status) {
					...
					$quote_data['shipping_cs_'.$key] = array(
						...
						'text'         => $cost == 0 ? $textarray[(int)$this->config->get('config_language_id')]['text_free'] : $this->currency->format($this->tax->calculate($cost, $custom_shipping['shipping_cs_tax_class_id'], 		$this->config->get('config_tax')), $this->session->data['currency'])
					);
				}
			}
		}
		...
		$method_data = array(
			'code'       => 'cs',
			'title'      => $textarray[(int)$this->config->get('config_language_id')]['shipping_name'],
			'quote'      => $quote_data,
