...
$(document).ready(function() {
	...
	$('input[type="tel"]').mask('+38(000)000-00-00', {placeholder: "000 000-00-00"});
	$('body').on('click','#sendContactForm',function(event){
		event.preventDefault();
		var id = '#callbackModal';
		var modal_title = 'Форма обратной связи';
	...
	$('#search input[name=\'search\']').parent().find('button').on('click', function() {
		var qweb = $('input[name="qweb"]').val();
		var url = $('base').attr('href') + qweb + 'index.php?route=product/search';

		var value = $('.new_header #search input[name=\'search\']').val();
	...
	$('#search input[name=\'search\']').on('keydown', function(e) {
		if (e.keyCode == 13) {
			$('.new_header #search input[name=\'search\']').parent().find('button').trigger('click');
		}
	});
...
var cart = {
	'add': function(product_id, quantity) {
		var data = new FormData();
		$(event.target).closest('.products-item').find('input[type="checkbox"]').each(function () {
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					var qcat2 = '';
					$('#q-dataL').html('');
					$('#dataL').html('<script>'+json['qcat'] + '</script>');
					Swal.fire({
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					$('#compare-total span').text(json['total']);
				}
			},
			error: function(xhr, ajaxOptions, thrownError) {
				alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
