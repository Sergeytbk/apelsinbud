...
class SlaSeoPro {
	...
	public function __construct($registry) {
		...
		if ($this->config->get('config_seo_url') && $this->config->get('sla_seo_pro_status')) {
			...
			$this->use_lang_prefix = $this->config->get('sla_seo_pro_use_lang_prefix');
			...
			$sla_seo_pro_lang_prefix = is_array($this->config->get('sla_seo_pro_lang_prefix'))?$this->config->get('sla_seo_pro_lang_prefix'):array();
			...
			if ($this->use_cache) {
				...
				if (!$this->cache_data) {
					...
					if ($this->use_lang_prefix) {
						...
						if ((int)$this->config->get('sla_seo_pro_use_lang_prefix_main')) {
							$sql_language = ' AND language_id = ' . (int)$this->config->get('sla_seo_pro_use_lang_prefix_main');
						}
					}
...
*/
	...
	public function prepareRoute() {
		...
		if (!$this->config->get('config_seo_url') && !$this->config->get('sla_seo_pro_status')) return;
		...
		if (!isset($this->request->get['_route_'])) {
			...
			if ($this->config->get('sla_seo_pro_prefix_product')) {
				$sla_seo_pro_prefix_product = trim($this->config->get('sla_seo_pro_prefix_product'));
			...
			if ($this->config->get('sla_seo_pro_prefix_category') && !$prefix) {
				$sla_seo_pro_prefix_category = trim($this->config->get('sla_seo_pro_prefix_category'));
			...
			if ($this->config->get('sla_seo_pro_prefix_brand') && !$prefix) {
				$sla_seo_pro_prefix_brand = trim($this->config->get('sla_seo_pro_prefix_brand'));
			...
			if (count($rows) == sizeof($parts)) {
				...
				foreach ($rows as $row) {
					if (isset($row['query']) && $row['query']) {
						...
						if (!$row['query']['language_id']) {
							$row['query']['language_id'] = $this->config->get('config_language_id');
						}
		...
		switch ($route) {
			case 'product/product':
				if (isset($data['product_id'])) {
					...
					$sla_seo_pro_utm = preg_split("/[\s,]+/",$this->config->get('sla_seo_pro_utm'));
			...
			case 'product/manufacturer/info':
				$prefix = $this->config->get('sla_seo_pro_prefix_brand');
				break;
...
//}
	...
	public function getLink($link) {
		...
		return 		$url_info['scheme'] . '://'
				...
				.	$url_info['path']
				.   (isset($url_info['query'])? '?' . str_replace('&','&amp;',$url_info['query']) : '');
	...
	public function validate() {
		...
		if (in_array(ltrim($this->request->server['REQUEST_URI'], '/'), array('sitemap.xml', 'robots.txt'))) {
			return;
		}
...
$ssl = false;
		...
		$seo = str_replace('&amp;', '&', $this->url->link($this->request->get['route'], $this->getQueryString(array('route','_route_')), $ssl));


        if($_SERVER['REMOTE_ADDR'] == '109.86.229.153'){
            //var_dump(rawurldecode($url));
            //var_dump(rawurldecode($seo));
            //var_dump($this->getQueryString(array('route','_route_')));
            //exit;
        }
...
//		exit;
	...
	private function getQueryString($exclude = array()) {
		...
		return ((http_build_query($need_get)));
	}
	...
	private function initLanguage($lang_code, $language_id) {
		...
		setcookie('language', $lang_code, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
		$this->registry->set('language', $language);
	}

	private function getKeyword($query) {
		if ($this->use_lang_prefix && (int)$this->config->get('sla_seo_pro_use_lang_prefix_main')) {
			$language_id = (int)$this->config->get('sla_seo_pro_use_lang_prefix_main');
		} else {
			$language_id = $this->language_id[$this->session->data['language']];
		}
